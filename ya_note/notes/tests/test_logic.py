...
class TestNoteLogic(BaseNotesTestSetup):
    ...
    def test_user_can_edit_own_note(self):
        ...
        self.assertEqual(self.own_note.title, self.EDITED_NOTE_TITLE)

    def test_user_can_delete_own_note(self):

        url_delete = reverse('notes:delete', args=(self.own_note.slug,))

        response = self.client_author.post(url_delete)

        self.assertEqual(response.status_code, HTTPStatus.FOUND)
        self.assertFalse(Note.objects.filter(slug=self.own_note.slug).exists())

    def test_user_cannot_edit_others_note(self):
        url_edit = reverse('notes:edit', args=(self.others_note.slug,))
        data_edit = {
        ...
        self.assertEqual(self.others_note.title, self.OTHERS_NOTE_TITLE)

    def test_user_cannot_delete_others_note(self):
        url_delete = reverse('notes:delete', args=(self.others_note.slug,))

        response = self.client_author.post(url_delete)

        self.assertEqual(response.status_code, HTTPStatus.NOT_FOUND)
        self.assertTrue(Note.objects.filter(slug=self.others_note.slug).exists())
